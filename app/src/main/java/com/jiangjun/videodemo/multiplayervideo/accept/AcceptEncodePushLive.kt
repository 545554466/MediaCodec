...
// type 码流
class AcceptEncodePushLive(
    var videoType: String,
    var width: Int,
    var height: Int,
    var callBack: AcceptSocketServer.scocketCallBack
) {
    private val TAG: String = "Record"
    private var mMediaCodec: MediaCodec? = null
    private var yuvByte: ByteArray? = null
    private var mvSocketServer: AcceptSocketServer? = null
    ...
    init {
        ...
        mvSocketServer = AcceptSocketServer(callBack)
    ...
    fun dealFrame(bb: ByteBuffer?, info: MediaCodec.BufferInfo) {
        ...
        when (getPictureType(bb, offset)) {
            ...
            H264_I -> {
                ...
                mvSocketServer?.sendData(newByte,1)
            }
            ...
            H265_I -> {
                ...
                val byte = ByteArray(info.size)
                bb?.get(byte)
                mvSocketServer?.sendData(byte,1)
            }
        }
    }
